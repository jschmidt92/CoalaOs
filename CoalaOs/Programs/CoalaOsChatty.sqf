...
fnCoala_startchatty = {
	...
	_textInput ctrlAddEventHandler ["KeyDown", {
		if (_this select 1 == 28) then {
			...
			if (_len > 0) then {
				[ctrlText _input, name player] remoteExec ["fnChatty_lastTextMessages", 0];
				_input ctrlSetText "";
			};
		};
	}];
	...
	[_width, _height, _processId] call fnChatty_drawTextFields;
	[] call fnChatty_reDisplayKeptMessages;
	[_processId] spawn fnChatty_receiveMessages;
};

fnChatty_drawTextFields = {
	params ["_width", "_height", "_processId"];
	_textY = -1.5;
	_textHeight = 1.5;
	_textWidth = _width - 2;
	_textX = 0.5;
